...
function ItemAccionesBarco({ item, onUpdated, onEliminar, notify }) {
  ...
  const handleFileChange = async (e) => {
    ...
    setLoading(true);
    try { const url = await api.subirAdjunto(item.id, file); await api.actualizarItem(item.id, { adjunto_url: url }); notify("Adjunto subido correctamente", "success"); onUpdated(); }
    catch (err) { alert("Error: " + err.message); }
...
function NuevaSolicitudModal({ barcoDefault, onClose, onSave, notify }) {
  ...
  const handleSave = async () => {
    ...
    if (sinSistema) return alert(`Seleccioná el sistema para el ítem "${sinSistema.descripcion || i + 1}"`);
    setSaving(true);
    try {
