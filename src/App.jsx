...
:root{
  ...
  --bg:#F0F4F8;--surface:#FFF;--surface2:#F5F7FA;--border:#D6E0ED;--border2:#C9D0D6;
  ...
  --action:#235C96;--tr:color 120ms,background-color 120ms,border-color 120ms;
}
...
.ni{display:flex;align-items:center;gap:9px;padding:7px 18px;font-size:12px;font-weight:500;cursor:pointer;color:rgba(255,255,255,.6);border-left:3px solid transparent;transition:all .12s;user-select:none;background:none;border-right:none;border-top:none;border-bottom:none;width:100%;text-align:left}
...
.sidebar-foot-btn{display:flex;align-items:center;gap:9px;width:100%;padding:7px 18px;background:none;border:none;cursor:pointer;font:500 12px/1.3 var(--sans);color:rgba(255,255,255,.55);transition:var(--tr);text-align:left}
...
/* MODALES */
.overlay{position:fixed;inset:0;background:rgba(33,51,99,.5);display:flex;align-items:flex-start;justify-content:center;z-index:100;padding:20px;overflow-y:auto}
.modal{background:var(--surface);border:1px solid var(--border);border-radius:12px;width:100%;max-width:800px;margin:auto;box-shadow:0 8px 32px rgba(33,51,99,.18)}
...
.fg input,.fg select,.fg textarea{background:var(--surface);border:1px solid var(--border);border-radius:var(--r);color:var(--text);font-family:var(--sans);font-size:13px;padding:8px 10px;outline:none;transition:border-color .15s;height:36px}
.fg textarea{resize:vertical;min-height:60px;height:auto;padding:8px 10px}
...
.toggle-btn{flex:1;padding:8px 12px;border-radius:var(--r);border:1px solid var(--border);background:var(--surface);color:var(--muted);font-family:var(--sans);font-size:12px;font-weight:600;cursor:pointer;transition:all .15s;text-align:center;text-transform:uppercase;letter-spacing:.5px}
...
.stat{background:var(--surface);border:1px solid var(--border);border-radius:var(--r2);padding:14px 16px;cursor:pointer;transition:all .15s}
.stat:hover{box-shadow:0 2px 8px rgba(33,51,99,.12)}
...
.items-table th{font-size:9px;font-weight:700;letter-spacing:.8px;color:var(--muted);text-transform:uppercase;padding:9px 12px;text-align:left;border-bottom:2px solid var(--navy);background:var(--surface2);white-space:nowrap}
...
.item-desc-cell{font-size:12px;color:var(--text)}
...
.notif{position:fixed;bottom:20px;right:20px;background:var(--surface);border:1px solid var(--border);border-left-width:3px;border-radius:var(--r2);padding:12px 16px;font-size:13px;z-index:300;max-width:340px;display:flex;align-items:center;gap:10px;box-shadow:0 4px 16px rgba(33,51,99,.15)}
    ...
    <div className="overlay" onClick={e => e.target === e.currentTarget && onClose()} style={{ zIndex: 200 }}>
      <div className="modal" style={{ maxWidth: 480 }}>
        <div className="mhdr" style={{ background: "var(--navy)", borderRadius: "12px 12px 0 0" }}>
          <div>
            <div className="mtitle" style={{ color: "#fff" }}>Marcar como Cumplido</div>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,.7)", marginTop: 3 }}>{item.numero_item} — {item.descripcion}</div>
          </div>
  ...
  return (
    <div className="overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        <div className="mhdr" style={{ background: "var(--navy)", borderRadius: "12px 12px 0 0" }}>
          <div>
            <div className="mtitle" style={{ color: "#fff" }}>Ítem {item.numero_item}</div>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,.7)", marginTop: 3 }}>{item.descripcion}</div>
          </div>
          <button className="mclose" style={{ color: "rgba(255,255,255,.7)" }} onClick={onClose}>✕</button>
        </div>
        <div className="mbody">
          ...
          {item.adjunto_url && <div className="mt12"><div style={{ fontSize: 9, fontWeight: 700, letterSpacing: 1, color: "var(--muted)", textTransform: "uppercase", marginBottom: 4 }}>Adjunto</div><a href={item.adjunto_url} target="_blank" rel="noreferrer" className="adjunto-link">📎 Ver archivo adjunto →</a></div>}
        </div>
        <div className="mftr">
          <button className="btn btn-ghost" onClick={onClose}>Cancelar</button>
...
function NuevaSolicitudModal({ barcoDefault, barcosPermitidos, onClose, onSave, notify }) {
  const [form, setForm] = useState({ barco: barcoDefault || "Golondrina de Mar", area: "", numero: "", fecha_emision: today(), emitido_por: "" });
  const [items, setItems] = useState([{ id: 1, descripcion: "", obs_capitan: "", tipo_reparacion: "", archivo: null }]);
  const [saving, setSaving] = useState(false);
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));
  const addItem = () => setItems(prev => [...prev, { id: Date.now(), descripcion: "", obs_capitan: "", tipo_reparacion: "", archivo: null }]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const itemsCreados = await Promise.all(itemsValidos.map(async (it, i) => {
        ...
        return { solicitud_id: sol.id, numero_item: `${form.numero}-${i + 1}`, descripcion: it.descripcion, obs_capitan: it.obs_capitan || null, tipo_reparacion: it.tipo_reparacion || null, estado: "pendiente", adjunto_url };
      }));
      await api.crearItems(itemsCreados);
      await api.enviarNotificacion({ barco: form.barco, area: form.area, numero: form.numero, fecha: fmtDate(form.fecha_emision), emitido_por: form.emitido_por, items: itemsCreados });
          ...
          <div className="form-grid-3">
            <FG label="Barco *">
              <select value={form.barco} onChange={e => set("barco", e.target.value)}>
                {barcosPermitidos.map(b => <option key={b}>{b}</option>)}
              </select>
            </FG>
      ...
      {expanded && (
        <div style={{ overflowX: "auto" }}>
          <table className="items-table">
            ...
            <tbody>
              {items.length === 0
                ? <tr><td colSpan={esBarco ? 9 : 9} style={{ textAlign: "center", padding: 20, color: "var(--muted2)" }}>Sin ítems</td></tr>
                : items.map(it => (
...
function PagePanel({ barco, notify, esBarco, onRefresh }) {
  ...
  return (
    <div>
      <div className="stats">
        <div className="stat" style={{ background: "#EEF4FB", borderColor: "#C5D8EE" }} onClick={() => { setFiltroEstado(""); setBusqueda(""); }}>
          <div className="stat-label">Total ítems</div><div className="stat-value" style={{ color: "var(--blue)" }}>{counts.total}</div>
        </div>
        <div className="stat" onClick={() => handleStatClick("pendiente")} style={{ background: filtroEstado === "pendiente" ? "#F5E6D0" : "#FEF3C7", borderColor: filtroEstado === "pendiente" ? "#D4943A" : "#FDE68A", outline: filtroEstado === "pendiente" ? "2px solid var(--warn)" : "none" }}>
          <div className="stat-label">Pendientes</div><div className="stat-value" style={{ color: "var(--warn)" }}>{counts.pendiente}</div>
        </div>
        <div className="stat" onClick={() => handleStatClick("en_proceso")} style={{ background: filtroEstado === "en_proceso" ? "#DBEAFE" : "#EFF6FF", borderColor: filtroEstado === "en_proceso" ? "#3B82F6" : "#BFDBFE", outline: filtroEstado === "en_proceso" ? "2px solid var(--blue)" : "none" }}>
          <div className="stat-label">En proceso</div><div className="stat-value" style={{ color: "var(--blue)" }}>{counts.en_proceso}</div>
        </div>
        <div className="stat" onClick={() => handleStatClick("cumplido")} style={{ background: filtroEstado === "cumplido" ? "#D1FAE5" : "#ECFDF5", borderColor: filtroEstado === "cumplido" ? "#10B981" : "#A7F3D0", outline: filtroEstado === "cumplido" ? "2px solid var(--accent2)" : "none" }}>
          <div className="stat-label">Cumplidos</div><div className="stat-value" style={{ color: "var(--accent2)" }}>{counts.cumplido}</div>
        </div>
        <div className="stat" onClick={() => handleStatClick("anulado")} style={{ background: filtroEstado === "anulado" ? "#E5E7EB" : "#F9FAFB", borderColor: filtroEstado === "anulado" ? "#9CA3AF" : "#E5E7EB", outline: filtroEstado === "anulado" ? "2px solid var(--muted)" : "none" }}>
      ...
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--navy)" }}>
        <div style={{ background: "var(--surface)", borderRadius: 12, padding: "40px 36px", width: "100%", maxWidth: 420, boxShadow: "0 8px 32px rgba(33,51,99,.25)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}>
            ...
            <div>
              <div style={{ fontWeight: 700, fontSize: 14, letterSpacing: 1, color: "var(--navy)", textTransform: "uppercase" }}>Reparaciones</div>
              <div style={{ fontSize: 10, color: "var(--muted)", letterSpacing: .5 }}>PL Offshore · Terra Mare Group</div>
            </div>
          </div>
          ...
          <button className="btn btn-primary" onClick={handleLogin} disabled={loading || !email || !password} style={{ width: "100%", justifyContent: "center", height: 42, fontSize: 13 }}>
        ...
        <nav className="sidebar">
          ...
          <div className="sidebar-foot">
            ...
            <div className="sidebar-foot-meta">
              <div>{userEmail}</div>
              <div style={{ marginTop: 2 }}>SSRR v1.6</div>
            </div>
          </div>
        </nav>
