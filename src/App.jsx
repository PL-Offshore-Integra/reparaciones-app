...
.stat{background:var(--surface);border:1px solid var(--border);border-radius:var(--r2);padding:14px 16px;cursor:pointer}
...
function NuevaSolicitudModal({ barcoDefault, barcosPermitidos, onClose, onSave, notify }) {
  const [form, setForm] = useState({ barco: barcoDefault||"Golondrina de Mar", numero:"", fecha_emision:today(), emitido_por:"", area:"", observaciones_generales:"" });
      ...
      <div className="modal modal-xl">
        ...
        <div className="mbody">
          ...
          <div className="form-grid-3">
            <FG label="Barco *"><select value={form.barco} onChange={e=>set("barco",e.target.value)}>{(barcosPermitidos||BARCOS).map(b=><option key={b}>{b}</option>)}</select></FG>
      ...
      <div className="app">
        <nav className="sidebar">
          ...
          <div className="sidebar-foot">
            ...
            <div className="sidebar-foot-meta"><div>{userEmail}</div><div style={{marginTop:2}}>SSRR v1.8</div></div>
          </div>
        </nav>
        <div className="main">
